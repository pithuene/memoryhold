...
import { ANDROID_EMULATOR_API_BASE_URL, DEFAULT_API_BASE_URL, MemoryholdApi, initialBackendUrl, isCapacitorRuntime, normalizeApiBaseUrl, saveBackendUrl, validateApiBaseUrl, type HealthResult, type Provider } from "./api";
import "./styles.css";
...
function shouldShowMessage(entry: any): boolean {
...
function isImage(a: any) { return a?.mimeType?.startsWith("image/") || /\.(png|jpe?g|gif|webp|svg)$/i.test(a?.filename ?? ""); }
function attachmentKind(filename = "", mimeType = "") { const l = filename.toLowerCase(); if (mimeType === "application/pdf" || l.endsWith(".pdf")) return "PDF"; if (mimeType.startsWith("image/") || /\.(png|jpe?g|gif|webp|svg)$/.test(l)) return "Image"; if (mimeType.startsWith("text/") || /\.(txt|md|csv|log)$/.test(l)) return "Document"; if (/\.(ts|tsx|js|jsx|py|rs|go|java|c|cpp|h|css|html|json)$/.test(l)) return "Code"; return "File"; }
function attachmentIcon(filename = "") { const l = filename.toLowerCase(); if (l.endsWith(".pdf")) return "PDF"; if (/\.(png|jpe?g|gif|webp|svg)$/.test(l)) return "IMG"; if (/\.(ts|tsx|js|jsx|py|rs|go|java|c|cpp|h|css|html|json|md)$/.test(l)) return "{}"; return "DOC"; }
...
function SettingsView(props: any) {
  ...
  const testConnection = async () => {
    ...
    if (status.ok) props.setConnectionCheckedAt(new Date().toISOString());
    setChecking(false);
  };
