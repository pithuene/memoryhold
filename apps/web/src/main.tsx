...
function renderContent(content: unknown): string {
  ...
  if (Array.isArray(content)) return content.map((block: any) => typeof block === "string" ? block : block?.type === "text" ? block.text ?? "" : block?.type === "thinking" || block?.type === "image" ? "" : block?.type === "toolCall" ? `[tool call: ${block.name}]` : JSON.stringify(block)).join("\n");
