...
function App() {
  ...
  const [oauthProviders, setOauthProviders] = useState<Array<{ id: string; name: string; authenticated: boolean }>>([]);
  ...
  const api = useMemo(() => new MemoryholdApi(apiBaseUrl, apiToken), [apiBaseUrl, apiToken]);
  ...
  const startOAuth = async (id: string) => { setErrorMessage(""); const result = await api.startOAuth(id); if (result.error) return showError(result.error); setLoginId(result.loginId); if (result.authUrl) { const browser = (window as any).Capacitor?.Plugins?.Browser; if (mobileRuntime && browser?.open) await browser.open({ url: result.authUrl }); else window.open(result.authUrl, "_blank"); } };
  ...
  return <div className={cn("app", sidebarCollapsed && "collapsed", mobileSidebarOpen && "mobile-sidebar-open")}>
    ...
    <Dialog.Root open={!!deleteCandidate} onOpenChange={(o) => !o && setDeleteCandidate(undefined)}><Dialog.Portal><Dialog.Overlay className="dialog-overlay"/><Dialog.Content className="dialog"><Dialog.Title>Delete conversation?</Dialog.Title><Dialog.Description>This will permanently delete “{deleteCandidate?.title}” and its attachments from the conversations folder.</Dialog.Description><div className="dialog-actions"><Dialog.Close asChild><button className="button secondary">Cancel</button></Dialog.Close><button className="button destructive" onClick={confirmDelete}>Delete</button></div></Dialog.Content></Dialog.Portal></Dialog.Root>
  </div>;
}
...
function SettingsView(props: any) {
  ...
  const testConnection = async () => {
    ...
    if (validation) return props.setConnectionStatus({ ok: false, url: backendDraft, error: validation });
    setChecking(true);
    const api = new MemoryholdApi(backendDraft, tokenDraft);
    const health = await api.health();
    if (!health.ok) { props.setConnectionStatus(health); setChecking(false); return; }
    try {
      await api.sessions();
      props.setConnectionStatus(health);
      props.setConnectionCheckedAt(new Date().toISOString());
    } catch (error) {
      props.setConnectionStatus({ ok: false, url: health.url, error: error instanceof Error ? error.message : String(error) });
    }
    setChecking(false);
  };
  const mode = props.mobileRuntime ? "Android remote client" : new URLSearchParams(location.search).get("memoryholdElectron") === "1" ? "Electron client" : "Web client";
  return <div className="settings-page"><div className="settings-inner"><div className="settings-hero"><h1>Settings</h1><p>Connect provider accounts and choose the default model for new messages.</p></div><section className="card"><h2>Server connection</h2><p>{props.mobileRuntime ? "Android devices cannot use desktop localhost. Use the emulator address, your server machine’s LAN IP, or a VPN/HTTPS URL." : "Choose which Memoryhold backend this client should use."}</p><div className="connection-meta"><small>Mode: {mode}</small>{props.connectionCheckedAt && <small>Last connected: {new Date(props.connectionCheckedAt).toLocaleString()}</small>}</div><div className="connection-row"><input className="backend-input" value={backendDraft} onChange={(e) => setBackendDraft(e.target.value)} placeholder="http://localhost:8787"/><div className="connection-actions"><button className="button secondary" onClick={testConnection} disabled={checking}>{checking ? "Checking…" : "Test"}</button><button className="button secondary" onClick={() => setBackendDraft(props.mobileRuntime ? ANDROID_EMULATOR_API_BASE_URL : DEFAULT_API_BASE_URL)}>Reset</button><button className="button" onClick={() => { props.saveBackend(backendDraft); props.saveToken(tokenDraft); }}>Save</button></div></div><label className="token-field"><span>Access token <small>Only needed when the server sets <code>MEMORYHOLD_ACCESS_TOKEN</code>.</small></span><input className="backend-input" type="password" value={tokenDraft} onChange={(e) => setTokenDraft(e.target.value)} placeholder="optional"/></label><small>Examples: local web <code>{DEFAULT_API_BASE_URL}</code>, Android emulator <code>{ANDROID_EMULATOR_API_BASE_URL}</code>, phone on LAN <code>http://192.168.1.23:8787</code>.</small>{props.connectionStatus && <p className={cn("connection-status", props.connectionStatus.ok ? "ok" : "bad")}>{props.connectionStatus.ok ? `Connected to ${props.connectionStatus.url}${props.connectionStatus.authRequired ? " · token required" : ""}` : `Cannot connect to ${props.connectionStatus.url}: ${props.connectionStatus.error}`}</p>}</section><section className="card"><h2>Accounts</h2><p>Credentials are stored on the backend in your conversations directory.</p>{props.oauthProviders.map((p: any) => <div className="account-row" key={p.id}><div><strong>{p.name}</strong><small>{p.authenticated ? "Connected" : "Not connected"}</small></div><button className={cn("button", p.authenticated ? "success" : "secondary")} onClick={() => props.startOAuth(p.id)}>{p.authenticated ? "Reconnect" : "Connect"}</button></div>)}{props.loginId && <><p><small>Browser opened. If callback does not complete, paste redirect URL/code:</small></p><textarea className="oauth-textarea" value={props.callbackInput} onChange={(e) => props.setCallbackInput(e.target.value)}/><button className="button" onClick={props.completeOAuth}>Complete login</button></>}</section><section className="card"><h2>Model defaults</h2><p>These settings are sent with each message and recorded in the conversation timeline.</p><div className="settings-grid"><select value={props.selectedProvider} onChange={(e) => props.setSelectedProvider(e.target.value)}>{props.providers.map((p: any) => <option key={p.id} value={p.id}>{p.id}</option>)}</select><select value={props.selectedModel} onChange={(e) => props.setSelectedModel(e.target.value)}>{props.providers.find((p: any) => p.id === props.selectedProvider)?.models.map((m: any) => <option key={m.id} value={m.id}>{m.name || m.id}</option>)}</select><select value={props.thinkingLevel} onChange={(e) => props.setThinkingLevel(e.target.value)}>{["off", "minimal", "low", "medium", "high"].map((l) => <option key={l} value={l}>thinking: {l}</option>)}</select></div></section></div></div>;
}
function ConnectionEmpty({ status, openSettings }: { status: Extract<HealthResult, { ok: false }>; openSettings: () => void }) { return <div className="empty connection-empty"><h1>Connect to Memoryhold server.</h1><p>Cannot reach {status.url}: {status.error}</p><button className="button" onClick={openSettings}>Open connection settings</button></div>; }
