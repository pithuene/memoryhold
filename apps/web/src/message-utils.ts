...
export function messageAttachments(message: any) {
  ...
  const context =
    renderContent(message.content).match(
      /<MEMORYHOLD_ATTACHMENT_CONTEXT>[\s\S]*?<\/MEMORYHOLD_ATTACHMENT_CONTEXT>/,
    )?.[0] ?? "";
  return Array.from(
    context.matchAll(/Attachment: (.+?) \((.+?), (attachments\/.+?)\)/g),
  ).map((m) => ({ filename: m[1], mimeType: m[2], relativePath: m[3] }));
}
...
export function attachmentKind(filename = "", mimeType = "") {
  ...
  if (mimeType.startsWith("image/") || /\.(png|jpe?g|gif|webp|svg)$/.test(l))
    return "Image";
  if (mimeType.startsWith("text/") || /\.(txt|md|csv|log)$/.test(l))
    return "Document";
  if (/\.(ts|tsx|js|jsx|py|rs|go|java|c|cpp|h|css|html|json)$/.test(l))
    return "Code";
  return "File";
}
...
export function attachmentIcon(filename = "") {
  ...
  if (/\.(ts|tsx|js|jsx|py|rs|go|java|c|cpp|h|css|html|json|md)$/.test(l))
    return "{}";
  return "DOC";
}
