...
async function jsonOrTextError(response: Response) {
  ...
  if (response.status === 401 || response.status === 403)
    throw new Error(
      text ||
        "Server rejected this client. Check the server access token in Settings.",
    );
...
export class MemoryholdApi {
  ...
  sessions() {
    return this.getJson<SessionMetadata[]>("/api/sessions");
  }
  createSession() {
    return this.sendJson<SessionMetadata>("/api/sessions", "POST", {});
  }
  session(slug: string) {
    return this.getJson<{
      metadata: SessionMetadata;
      entries: SessionTreeEntry[];
    }>(`/api/sessions/${encodeURIComponent(slug)}`);
  }
  renameSession(slug: string, title: string) {
  ...
  deleteSession(slug: string) {
    return this.deleteJson<{ ok: true }>(
      `/api/sessions/${encodeURIComponent(slug)}`,
    );
  }
  providers() {
    return this.getJson<Provider[]>("/api/providers");
  }
  oauthProviders() {
    return this.getJson<
  ...
  startOAuth(id: string) {
    return this.sendJson<any>(
      `/api/oauth/${encodeURIComponent(id)}/start`,
      "POST",
      {},
    );
  }
  completeOAuth(loginId: string, code: string) {
    return this.sendJson<any>(
      `/api/oauth/${encodeURIComponent(loginId)}/complete`,
      "POST",
      { code },
    );
  }
  oauthStatus(loginId: string) {
    return this.getJson<any>(
      `/api/oauth/${encodeURIComponent(loginId)}/status`,
    );
  }
  ...
  async uploadAttachments(slug: string, files: File[]) {
    ...
    const response = await fetch(
      this.url(`/api/sessions/${encodeURIComponent(slug)}/attachments`),
  ...
  async sendMessage(slug: string, body: unknown) {
    return this.sendJson<any>(
      `/api/sessions/${encodeURIComponent(slug)}/messages`,
      "POST",
      body,
    );
  }
  ...
  eventsUrl(slug: string) {
    const url = new URL(
      this.url(`/api/sessions/${encodeURIComponent(slug)}/events`),
    );
  ...
  attachmentUrl(slug: string, relativePath: string) {
    const url = new URL(
      this.url(`/api/sessions/${encodeURIComponent(slug)}/${relativePath}`),
    );
    if (this.token) url.searchParams.set("access_token", this.token);
    return url.toString();
  }
