import { rmSync, mkdirSync, cpSync, writeFileSync, realpathSync } from 'node:fs';
...
const canvasBinaryPackage = realpathSync(join(root, `node_modules/.pnpm/@napi-rs+${canvasBinaryName}@1.0.0/node_modules/@napi-rs/${canvasBinaryName}`));
mkdirSync(join(staging, 'node_modules/@napi-rs'), { recursive: true });
cpSync(canvasPackage, join(staging, 'node_modules/@napi-rs/canvas'), { recursive: true });
cpSync(canvasBinaryPackage, join(staging, `node_modules/@napi-rs/${canvasBinaryName}`), { recursive: true });
...
await esbuild({
  ...
  target: 'node22',
  format: 'esm',
  banner: { js: "import { createRequire as __mhCreateRequire } from 'module'; const require = __mhCreateRequire(import.meta.url);" },
  external: ['fsevents', '@napi-rs/canvas', '@napi-rs/canvas-linux-arm64-gnu', '@napi-rs/canvas-linux-arm64-musl', '@napi-rs/canvas-linux-x64-gnu', '@napi-rs/canvas-linux-x64-musl'],
  sourcemap: false,
});
